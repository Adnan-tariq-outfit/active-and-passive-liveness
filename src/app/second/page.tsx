...
export default function OptimizedFaceLiveness() {
  ...
  // Fast blink detection - immediate response
  const handleBlinkDetection = useCallback(
    (landmarks: FaceLandmark[]) => {
      ...
      const eyesClosed = avgEAR < 0.22; // Lower threshold for closed
      const eyesOpen = avgEAR > 0.23; // Higher threshold for open

      // Detect blink: eyes were open, then closed, then open again
      if (eyesClosed && state.lastEyeAspectRatio > 0.23) {
        // Eyes just closed - mark as closing
        state.consecutiveFrames = 1;
      } else if (
        eyesOpen &&
        state.lastEyeAspectRatio < 0.22 &&
        state.consecutiveFrames > 0
      ) {
        // Eyes just opened after being closed - this is a blink!
        state.blinkCount++;
        state.consecutiveFrames = 0;

        // Update status immediately
        if (state.blinkCount >= 2) {
          // Capture photo immediately after 2 blinks
          if (!state.capturedPhotos.blink) {
        ...
        // Reset if eyes are open and no blink detected
        state.consecutiveFrames = 0;
      }
  ...
  // Smile detection - only when face is straight (not turning)
  const handleSmileDetection = useCallback(
    (landmarks: FaceLandmark[]) => {
      const state = detectionStateRef.current;
      if (state.isTransitioning || state.smileDetected) return;

      // Check head pose - if head is turning, don't detect smile
      const headPose = calculateHeadPose(landmarks);
      const isHeadStraight = headPose > -0.1 && headPose < 0.1; // Face should be relatively straight

      // Use the simple calculateMouthOpenness approach
      const mouthOpenness = calculateMouthOpenness(landmarks);

      // Smile threshold - reasonable range
      // A smile has mouth openness between 0.01 and 0.08
      // Too low (< 0.01) = closed mouth, too high (> 0.08) = open mouth (not a smile)
      const isSmiling = mouthOpenness > 0.01 && mouthOpenness < 0.08;

      // Only detect smile if:
      // 1. Mouth openness is in smile range
      // 2. Head is straight (not turning left or right)
      const validSmile = isSmiling && isHeadStraight;

      if (validSmile) {
        // Increment consecutive frames
        ...
        // Reset if smile is not valid
        state.smileConsecutiveFrames = 0;

        // Show appropriate message
        if (!isHeadStraight) {
          setStatus("Please keep your face straight and smile 😊");
        } else {
      ...
      if (state.frameCount % 30 === 0) {
        console.log("😊 Smile Detection:", {
          ...
          RESULT: validSmile ? "✅ SMILING" : "❌ NOT SMILING",
        });
      }
    },
    [capturePhoto, transitionToNextStep]
  );
  // Optimized anti-spoofing calculation
  const calculateAntiSpoofing = useCallback(
    (landmarks: FaceLandmark[]): number => {
      const state = detectionStateRef.current;

      if (state.lastLandmarks && state.frameCount % 3 === 0) {
        // Check every 3 frames for performance
      ...
      if (state.frameCount % 5 === 0) {
        // Update landmarks every 5 frames
        state.lastLandmarks = landmarks.map((l) => ({ ...l }));
      }
      state.frameCount++;

      const depthScore = Math.min(state.depthVariationScore * 10, 100);
      const motionScoreNorm = Math.min(state.motionScore * 5, 100);

      return Math.round(depthScore * 0.5 + motionScoreNorm * 0.5);
    },
    []
  );
  const completeVerification = useCallback(async () => {
    ...
    const antiSpoofingScore = calculateAntiSpoofing(state.lastLandmarks || []);
    ...
    try {
      ...
      console.log("Anti-spoofing score:", antiSpoofingScore);
    } catch (err) {
  ...
  const onResults = useCallback(
    (results: FaceMeshResults) => {
      ...
      // Anti-spoofing (only when not complete)
      if (currentStep !== "COMPLETE") {
        calculateAntiSpoofing(landmarks);
      }
    },
    [
      step,
